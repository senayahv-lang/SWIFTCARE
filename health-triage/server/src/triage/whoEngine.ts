/**
 * WHO-aligned triage logic (simplified for community health worker use)
 * References:
 * - WHO Malaria Guidelines 2023 (test-treat-track)
 * - WHO IMCI / iCCM fever assessment
...
function hasSevereMalariaSigns(signs: MalariaInput['severeSigns']): boolean {
  return Object.values(signs).some(Boolean);
}

export function assessMalaria(input: MalariaInput): MalariaAssessment {
  const severe = hasSevereMalariaSigns(input.severeSigns);
